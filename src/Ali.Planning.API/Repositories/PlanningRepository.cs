...
namespace Ali.Planning.API.Repositories
{
        ...
        IPlanningRepository
    {
        ...
        public IQueryable<PlanModel> GetEmployeePlansWithProjectName(int employeeId)
        {
           return _context.Plannings.Where(e => e.EmployeeId == employeeId).Join(_context.Projects, p => p.ProjectId, j => j.Id, (p, j) => new { plan = p, proj = j})
